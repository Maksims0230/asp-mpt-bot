using ASPTelegramMPTBot.Models;
...
using ASPTelegramMPTBot.Models.Firebase;
using ASPTelegramMPTBot.Models.GetUpdate;
using ASPTelegramMPTBot.Models.Users;
...
namespace ASPTelegramMPTBot.Controllers
{
    ...
    public static class DataBot
    {
        ...
        public static GetUpdate getUpdates(long ID_Offset)
        {
            var json_str = GetResponce_in_Json(ID_Offset);
            if (json_str.Trim() != "")
                return JsonConvert.DeserializeObject<GetUpdate>(json_str);
            else
                return new GetUpdate() { ok = false, result = null };
        }
    ...
    public class Programms
    {
        ...
        public static List<string> Commands
        {
            ...
            set
            {
                OldCountCommands = value.Count;
                _commands = value;
                if (ActualCountCommands - OldCountCommands == 2)
                    UpdateCommands = true;
                else
                    UpdateCommands = false;
            }
        }
        public static int ActualCountCommands { get { return Commands.Count; } private set { } }
        public static int OldCountCommands { get; private set; }
        public static bool UpdateCommands { get; private set; }
    }
    ...
    public class HomeController : Controller
    {
        ...
        public static Thread notifications = new Thread(() =>
        {
            thread_start:
            try
            {
        ...
        public Thread main_thread = new Thread(() =>
        {
            ...
            try
            {
                if (NetworkInterface.GetIsNetworkAvailable())
                {
                    ...
                    if (notifications.ThreadState == System.Threading.ThreadState.Stopped)
                        notifications.Start();

                    var updates = DataBot.getUpdates(0);

                    long id_offset = Firebase.set_get_id ?? 0;
                    ...
                    while (true)
                    {
                        ...
                        if (updates != null && updates.ok && updates.result.Count > 0)
                        {
                            ...
                            updates.result.ToList().ForEach(x =>
                            {
                                if (x.message != null)
                                {
                                    Programms.Commands.Add($"(wait) OffsetID: {id_offset}; UpdateID: {x.update_id}; ChatID: {x.message.chat.id}; -> {x.message.text}");
                                    try
                                    {
                                        if (x.message.entities != null && x.message.entities.Last().type == EntitiesType.bot_command.ToString())
                                        {
                                            ...
                                            if (str.Contains("setgroup"))
                                            {
                                                ...
                                                if (gr0.Any(z => z == gr1))
                                                {
                                                    if (users.All(u => u.from.id != x.message.from.id))
                                                    {
                                                        ...
                                                        Firebase.InsertUser(new User() { from = x.message.from, group = new string(x.message.text.Skip(10).ToArray()) });
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        Firebase.UpdateUser(users.Where(u => u.from.id == x.message.from.id).First());
                                            ...
                                            else if (str.Contains("lessonsweek") && str.Trim().Length > 13)
                                            {
                                                ...
                                                switch (str)
                                                {
                                                    ...
                                                    case "typeweek":
                                                        ...
                                                        break;
                                                    case "notifon":
                                                        if (Users.users.Any(z => x.message.from.id == z.from.id))
                                                        {
                                                            var users = Users.users;
                                                            ...
                                                            Firebase.UpdateUser(users.Where(z => x.message.from.id == z.from.id).First());
                                            ...
                                            Programms.Commands.Add($"(ok) OffsetID: {id_offset}; UpdateID: {x.update_id}; ChatID: {x.message.chat.id}; -> {x.message.text}");
                                            //Console.ResetColor();
                                            goto endall;
        ...
        {
            main_thread.Start();
            return View(Programms.Commands);
        }
